...
class OrbitControls extends EventDispatcher {
  findCamera(obj) {
    for (let i = 0; i < obj.children.length; i += 1) {
      if (obj.children[i].isCamera) {
        return obj.children[i];
      }

      const cam = this.findCamera(obj.children[i]);
      if (cam) {
        return cam;
      }
    }
  ...
  constructor(object, domElement) {
    ...
    this.camera = this.findCamera(this.object);
    ...
    const pan = (function () {
      ...
      return function (deltaX, deltaY) {
        ...
        if (scope.camera.isPerspectiveCamera) {
          ...
          // orthographic
          panLeft(deltaX * (scope.camera.right - scope.camera.left) / scope.camera.zoom / element.clientWidth, scope.object.matrix);
          panUp(deltaY * (scope.camera.top - scope.camera.bottom) / scope.camera.zoom / element.clientHeight, scope.object.matrix);
        } else {
